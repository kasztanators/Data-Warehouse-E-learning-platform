...
with open(filename_enrol, "w", newline="") as csvfile,open(task_res, "w", newline="") as task_csv,open(filename_quiz_res, "w", newline="") as quiz_csv:
    ...
    for i in range(1, enrollments_rows+1):
        ...
        date_start = start_date + timedelta(days=random.randint(0, 365))
        ...
        for j in range(1, 5):
            grade = random.randint(2, 5)
            sum+=grade
            ass_submission_date = date_start + timedelta(days=random.randint(0, 100))
            while True:
                ass_submission_date = date_start + timedelta(days=random.randint(0, 100))
        ...
        writer_enr.writerow({
            "ID": i,
            "Date of start": date_start.strftime("%Y-%m-%d"),
            "Grade": round((sum / 8) * 2) / 2,
            "Percentage of completion": percent_complete,
            "Completion date": date_complete.strftime("%Y-%m-%d")if date_complete else "",
